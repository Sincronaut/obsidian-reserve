...
$grid_items       = $attributes['gridItems'] ?? array();
$variant          = $attributes['variant'] ?? 'standard';

$theme_uri    = get_stylesheet_directory_uri();
$main_img_src = ! empty( $main_image_url ) ? ( 0 === strpos( $main_image_url, 'http' ) ? $main_image_url : $theme_uri . $main_image_url ) : '';
...
<section <?php echo get_block_wrapper_attributes( array( 'class' => 'featured-fleet-section variant-' . esc_attr( $variant ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
		...
		<div class="fleet-main-card">
			...
			<?php if ( $main_img_src ) : ?>
				<div class="fleet-main-image">
					<img src="<?php echo esc_url( $main_img_src ); ?>" alt="<?php echo esc_attr( wp_strip_all_tags( $main_title ) ); ?>" />
				</div>
			<?php endif; ?>
		</div>
