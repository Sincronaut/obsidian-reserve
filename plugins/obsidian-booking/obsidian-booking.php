...
function obsidian_booking_maybe_flush_rewrites() {
	...
	if ( OBSIDIAN_BOOKING_VERSION === $stored_version ) {
		return;
	}
...
/**
 * Render the booking modal markup.
 *
 * @return void
 */
 ...
 * @return mixed
 */
function obsidian_sync_acf_to_post_title( $value, $post_id ) {
	if ( ! $value ) {
		return $value;
	}
...
add_filter( 'acf/update_value/name=location_name', 'obsidian_sync_acf_to_post_title', 10, 2 );
add_filter( 'acf/update_value/name=car_name', 'obsidian_sync_acf_to_post_title', 10, 2 );
 ...
 * @return mixed
 */
function obsidian_sync_post_title_to_acf( $value, $post_id ) {
	// If the post exists, use its actual title.
	$post = get_post( $post_id );
	if ( $post && ( 'car' === $post->post_type || 'location' === $post->post_type ) ) {
		// Don't override if the title is "Auto Draft" (new post).
		if ( 'Auto Draft' !== $post->post_title && ! empty( $post->post_title ) ) {
			return $post->post_title;
		}
	}
	return $value;
}
add_filter( 'acf/load_value/name=location_name', 'obsidian_sync_post_title_to_acf', 10, 2 );
add_filter( 'acf/load_value/name=car_name', 'obsidian_sync_post_title_to_acf', 10, 2 );
